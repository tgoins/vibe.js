...
bot.on('ready', () => {
  ...
  if(musicTextChannelId) {
    BotState.textChannel = server.channels.find(chn => chn.id == musicTextChannelId && chn.type === "text"); //The text channel the bot will use to announce stuff
  

    if(!BotState.textChannel) {
      throw "Couldn't find text channel with id " + musicTextChannelId + "' in server '" + serverName + "'";
    }
  }
