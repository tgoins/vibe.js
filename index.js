...
bot.on('message', msg => {
  ...
  if (args.length - 1 < commandObj.parameters.length) {
    return msg.reply(`Missing arguments. Expected ${commandObj.parameters.length} but received ${args.length - 1}. ` +
    `The '${commandObj.name}' command expects these arguments: ${commandObj.parameters.join(', ')}.`
    )
      .then(m => {
        if (autoDeleteMessage) {
          setTimeout(() => m.delete(), autoDeleteMessageDelay)
  ...
  if (commandObj.requiresVoiceChannel) {
    voiceChannel
      .join()
      .then(connection => {
        BotState.voiceConnection = connection
...
process.on('uncaughtException', (err) => {
  if (err.code === 'ECONNRESET') {
    console.log('ECONNRESET Error, RIP')
    console.log(err.stack)
  } else {
    console.log(err)
    console.log(err.stack)
    // Shutdown bot. Will automatically restart.
    process.exit(0)
  }
})
